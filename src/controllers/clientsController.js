...
class ClientController {
    ...
    static async findOne(req, res) {
        try {
            ...
            res.status(200).json(ApiResponse.returnSucess(await Clients.findById(req.params.id)));
        } catch (e) {
    ...
    static async findAll(req, res) {
        try {
            ...
            res.status(200).json(ApiResponse.returnSucess(await Clients.find(client)));
        } catch (e) {
            if (e.name === 'TokenExpiredError') {
                res.status(401).json(ApiResponse.tokenExpired())
            } else if (e instanceof Jwt.JsonWebTokenError) {
    ...
    static async update(req, res) {
        try {
            ...
            if (db) {
                ...
                res.status(400).json(ApiResponse.returnError('Nenhum dado atualizado, verifique os filtros e tente novamente.'))
            }
        } catch (e) {
            if (e.name === 'TokenExpiredError') {
                res.status(401).json(ApiResponse.tokenExpired())
    ...
    static async delete(req, res) {
        try {
            ...
            if (db) {
                ...
                res.status(400).json(ApiResponse.returnError('Nenhum dado excluido, verifique os filtros e tente novamente'))
            }
        } catch (e) {
            if (e.name === 'TokenExpiredError') {
                res.status(401).json(ApiResponse.tokenExpired())
